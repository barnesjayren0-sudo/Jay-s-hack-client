...
/** Max 3-tick sneak pulses — never sticky; off while Scaffold on. */
public class SafeWalk extends Module {
    ...
    public SafeWalk() {
        super("SafeWalk", "Silent edge sneak", Category.MOVEMENT);
    }
    ...
    public void onTick() {
        ...
        int sdx = mx > 0.03 ? 1 : (mx < -0.03 ? -1 : 0);
        int sdz = mz > 0.03 ? 1 : (mz < -0.03 ? -1 : 0);
        if (sdx != 0 || sdz != 0) {
            if (mc.theWorld.isAirBlock(below.add(sdx, 0, sdz))) edge = true;
        }
